...
export default async function HomePage({ searchParams }: Props) {
  ...
  try {
    ...
    articles = (rawArticles ?? []) as unknown as ArticleWithAuthor[]
  } catch (err) {
    console.warn('Homepage articles query failed, falling back to mock data:', err)
    articles = MOCK_ARTICLES.filter(a => a.status === 'published').map(a => ({
      ...a,
      author: a.author,
      category: a.category,
  ...
  // Hero slides: featured first, then top by views, fill from latest
  const featured   = articles.filter(a => (a as any).featured)
  const heroSlides = [
    ...featured,
    ...[...articles].sort((a, b) => b.views - a.views).filter(a => !featured.find(f => f.article_id === a.article_id)),
  ].slice(0, 7)
  ...
  try {
    const { data: rawJournalists, error } = await supabase
      ...
      .limit(3) as any
    if (error) throw error
    journalists = (rawJournalists ?? []) as unknown as JournalistRow[]
  } catch (err) {
    console.warn('Homepage journalists query failed, falling back to mock data:', err)
    journalists = MOCK_USERS.filter(u => u.role === 'journalist').slice(0, 3).map(u => ({
      user_id: u.user_id,
      name: u.name,
  ...
  try {
    const { data: rawCategories, error } = await supabase.from('categories').select('category_id, name') as any
    if (error) throw error
  ...
  // Filter display articles based on category parameter
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
        ...
        <div className="bg-[#0a1628] text-white py-2 overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 flex items-center gap-3 text-sm">
            ...
            <div className="flex gap-8 overflow-hidden whitespace-nowrap text-white/70">
              {trending.map(a => (
                <Link key={a.article_id} href={`/article/${a.slug}`}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 grid lg:grid-cols-[1fr_300px] gap-8 w-full">
            ...
            <>
              ...
              <div className="space-y-4">
                {spotlight.map(article => (
                  <div key={article.article_id} className="bg-white dark:bg-gray-800/40 border dark:border-gray-800/60 rounded-xl shadow-sm p-4 flex gap-4 items-start hover:shadow-md transition-all hover:-translate-y-0.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2">{article.content.substring(0, 100)}...</p>
                      {article.author && (
                        <div className="flex items-center gap-1.5 mt-2">
                          {article.author.profile_image && (
                            <Image src={article.author.profile_image} alt={article.author.name} width={20} height={20} className="rounded-full object-cover" />
                          )}
                          <span className="text-xs text-gray-400 dark:text-gray-500">{article.author.name} — Freelance Journalist</span>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <aside className="space-y-5">
          {trending.length > 0 && (
            <div className="bg-white dark:bg-gray-800/40 border dark:border-gray-800/60 rounded-xl shadow-sm overflow-hidden transition-colors">
              <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800/80 border-b-2 border-blue-600">
                <h3 className="text-sm font-extrabold text-gray-900 dark:text-white uppercase tracking-wider">🔥 Trending Now</h3>
              </div>
              {trending.map((a, i) => (
                <div key={a.article_id} className="flex items-start gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                  ...
                  <div>
                    <h5 className="text-sm font-semibold text-gray-800 dark:text-gray-200 leading-snug">
                      <Link href={`/article/${a.slug}`} className="hover:text-blue-600 dark:hover:text-blue-400">{a.title}</Link>
                    </h5>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">👁 {formatNumber(a.views)} · {a.category?.name}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-gray-800/40 border dark:border-gray-800/60 rounded-xl shadow-sm overflow-hidden transition-colors">
            <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800/80 border-b-2 border-blue-600">
              <h3 className="text-sm font-extrabold text-gray-900 dark:text-white uppercase tracking-wider">📂 Categories</h3>
            </div>
            {categories.map(cat => (
              <div key={cat.category_id} className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                <div>
                  <Link href={`/?category=${cat.name}`} className="text-sm font-semibold text-gray-800 dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400">{cat.name}</Link>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{articles.filter(a => a.category?.name === cat.name).length} articles</p>
                </div>
          ...
          {journalists.length > 0 && (
            <div className="bg-white dark:bg-gray-800/40 border dark:border-gray-800/60 rounded-xl shadow-sm overflow-hidden transition-colors">
              <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800/80 border-b-2 border-blue-600">
                <h3 className="text-sm font-extrabold text-gray-900 dark:text-white uppercase tracking-wider">🏆 Top Journalists</h3>
              </div>
              {journalists.map(j => (
                <Link key={j.user_id} href={`/journalists/${j.user_id}`} className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                  {j.profile_image ? (
              ...
              <Link href="/leaderboard" className="block text-center text-xs font-semibold text-blue-600 dark:text-blue-400 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
